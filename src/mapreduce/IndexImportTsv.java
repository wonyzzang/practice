...
public class IndexImportTsv {
	...
	static boolean doesTableExist(String tableName) throws IOException {
		return hbaseAdmin.tableExists(Bytes.toBytes(tableName));
	}

	static void createTable(Configuration conf, String tableName) throws IOException {
		...
		for (String aColumn : columns) {
			if (TsvParser.ROWKEY_COLUMN_SPEC.equals(aColumn))
				continue;
			// we are only concerned with the first one (in case this is a
			// cf:cq)
	...
	private static void usage(final String errorMsg) {
		...
		String usage = "Usage: " + NAME + " -Dimporttsv.columns=a,b,c <tablename> <inputdir>\n" + "\n"
