...
/**
 * A column value is composed of many values separated using some known
 * separator. Part of the column value to be indexed. This class specified how
 * to get that value part. Takes the separator so as to split the value and the
 * value position in the split. Note that the position index starts from '1'
 */
public class SeparatorPartition implements ValuePartition {
	...
	public byte[] getPartOfValue(byte[] value) {
		...
		if (separatorOccurences == this.position - 1) {
			...
			if (sepLastKnownPosition == -1) {
				...
				System.arraycopy(value, sepLastKnownPosition + separator.length, valuePart, 0, valuePart.length);
			}
			return valuePart;
		}
		return valuePart;
	...
	public int compareTo(ValuePartition vp) {
		...
		if (diff == 0)
			return this.position - sp.position;
		return diff;
	}
	...
	public boolean equals(Object that) {
		if (this == that) {
			...
			return Bytes.compareTo(this.separator, sp.getSeparator()) == 0 && this.position == sp.getPosition();
		}
		return false;
	}
