...
public class ColumnQualifier implements WritableComparable<ColumnQualifier> {
	...
	public ColumnQualifier(String cf, String qualifier, ValueType type, int maxValueLength, ValuePartition vp) {
		this(Bytes.toBytes(cf), Bytes.toBytes(qualifier), type, maxValueLength, vp);
	}

	public ColumnQualifier(byte[] cf, byte[] qualifier, ValueType type, int maxValueLength, ValuePartition vp) {
		this.cfBytes = cf;
	...
	public int hashCode() {
		...
		if (valuePartition != null)
			result ^= valuePartition.hashCode();
		return result;
	}
