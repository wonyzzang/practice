...
/**
 * An IndexSpecification should have a unique name and can be created on 1 or
 * more columns (Here column refers to columnfamily + qualifier) For each of
 * such column a ColumnQualifier is provided which takes the column details.
 * This includes the column family name and qualifier name. The additional
 * columns are those columns in the main table whose value also will be captured
 * in the secondary index table. Index Specfication name should not start with
 * '.' and '-'. Can contain alphanumerics and '-','.','_'.
 */
...
public class IndexSpecification implements WritableComparable<IndexSpecification> {
	...
	/**
	 * @param name
	 *            should not start with '.' and '-'. Can contain alphanumerics
	 *            and '-','.','_'.
	 * @throws IllegalArgumentException
	 *             if invalid table name is provided
	 */
	 ...
	 * @param qualifier
	 * @param type
	 *            - If type is specified as null then by default ValueType will
	 *            be taken as String.
	 * @param maxValueLength
	 * @throws IllegalArgumentException
	 *             If column family name and/or qualifier is null or blanks.
	 *             <br/>
	 *             If column family name starts with '.',contains control
	 *             characters or colons.
	 * @see ValueType
	 */
	public void addIndexColumn(HColumnDescriptor cf, String qualifier, ValueType type, int maxValueLength)
			throws IllegalArgumentException {
	 ...
	 * @throws IllegalArgumentException
	 */
	public void addIndexColumn(HColumnDescriptor cf, String qualifier, ValuePartition vp, ValueType type,
			int maxValueLength) throws IllegalArgumentException {
		checkForType(type);
		isValidFamilyAndQualifier(cf, qualifier);
		maxValueLength = getMaxLength(type, maxValueLength);
		ColumnQualifier cq = new ColumnQualifier(cf.getNameAsString(), qualifier, type, maxValueLength, vp);
		isNotDuplicateEntry(cq);
		formMinTTL(cf);
		formMaxVersions(cf);
		internalAdd(cq);
	...
	private void isNotDuplicateEntry(ColumnQualifier c) {
		if (this.getIndexColumns().contains(c)) {
			throw new IllegalArgumentException(
					"Duplicate column family and qualifier " + "combination should not be present.");
		}
	}
	...
	public void readFields(DataInput in) throws IOException {
		...
		for (int i = 0; i < indexColsSize; i++) {
			...
			// Need to revisit this place. May be some other valid value though
			// invalid
			// comes up.
			try {
				cq.readFields(in);
	...
	public boolean contains(byte[] family, byte[] qualifier) {
		...
		for (ColumnQualifier qual : indexColumns) {
			if (Bytes.equals(family, qual.getColumnFamily()) && Bytes.equals(qualifier, qual.getQualifier())) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Return the minimum of the timeToLive specified for the column families in
	 * the specifed index
	 * 
	 * @return
	 */
	...
	/**
	 * Return the minimum of the maxVersion specified for the column families in
	 * the specified index
	 * 
	 * @return
	 */
