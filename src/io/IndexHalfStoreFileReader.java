...
/**
 * A facade for a {@link org.apache.hadoop.hbase.io.hfile.HFile.Reader} that
 * serves up either the top or bottom half of a HFile where 'bottom' is the
 * first half of the file containing the keys that sort lowest and 'top' is the
 * second half of the file with keys that sort greater than those of the bottom
 * half. The top includes the split files midkey, of the key that follows if it
 * does not exist in the file.
 * <p>
 * This type works in tandem with the {@link Reference} type. This class is used
 * reading while Reference is used writing.
 * <p>
...
public class IndexHalfStoreFileReader extends StoreFile.Reader {
	...
	public HFileScanner getScanner(final boolean cacheBlocks, final boolean pread, final boolean isCompaction) {
		...
		return new HFileScanner() {
			...
			public ByteBuffer getKey() {
				...
				// If it is top store file replace the StartKey of the Key with
				// SplitKey
			...
			private ByteBuffer getChangedKey(KeyValue kv) {
				...
				KeyValue newKv = new KeyValue(newRowkey, kv.getFamily(), kv.getQualifier(), kv.getTimestamp(),
			...
			private byte[] getNewRowkeyByRegionStartKeyReplacedWithSplitKey(KeyValue kv) {
				...
				int lenOfRegionStartKey = Bytes.toShort(value, 0); // 1st 2
																	// bytes
																	// length of
																	// the
																	// region
			...
			public ByteBuffer getValue() {
				...
				// If it is top store file change the value corresponding to the
				// changed key like
				// [first 2 bytes]StartKey length replace with SplitKey length
				// [last 2 bytes]ActualRowKey offset add with difference of
				// SplitKey & StartKey
			...
			private byte[] getChangedValue(byte[] value) {
				if (value.length == 0)
					return value; // The value can be empty when the KV type is
									// DELETE.
				...
				System.arraycopy(Bytes.toBytes((short) splitRow.length), 0, changedValue, 0, ROW_KEY_LENGTH);
			...
			public KeyValue getKeyValue() {
				...
				// If it is a top store file change the StartKey with SplitKey
				// in Key
				...
				KeyValue changedKv = new KeyValue(changedKey, kv.getFamily(), kv.getQualifier(), kv.getTimestamp(),
			...
			public boolean next() throws IOException {
				...
				// TODO check what will be returned when next moves the cursor
				// to the last entry
				// in the file
				while (true) {
					...
					// We need to check whether the current KV pointed by this
					// reader is corresponding to
					// this split or not.
					// In case of top store file if the ActualRowKey >= SplitKey
					// In case of bottom store file if the ActualRowKey <
					// Splitkey
			...
			public boolean seekBefore(byte[] key, int offset, int length) throws IOException {
				if (top) {
					byte[] fk = getFirstKey();
					// This will be null when the file is empty in which we can
					// not seekBefore to any key
					if (fk == null) {
						return false;
					}
			...
			public boolean seekTo() throws IOException {
				...
				while (true) {
					// We need to check the first occurrence of satisfying the
					// condition
					// In case of top store file if the ActualRowKey >= SplitKey
					// In case of bottom store file if the ActualRowKey <
					// Splitkey
			...
			public int seekTo(byte[] key, int offset, int length) throws IOException {
				if (top) {
					...
					int seekTo = delegate.seekTo(replacedKey.getBuffer(), replacedKey.getKeyOffset(),
							replacedKey.getKeyLength());
					return seekTo;
					/*
					 * if (seekTo == 0 || seekTo == -1) { return seekTo; } else
					 * if (seekTo == 1) { boolean next = this.next(); }
					 */
				} else {
					if (getComparator().compare(key, offset, length, splitkey, 0, splitkey.length) >= 0) {
	...
	private boolean isSatisfiedMidKeyCondition(KeyValue kv) {
		if (KeyValue.isDelete(kv.getType()) && kv.getValue().length == 0) {
			// In case of a Delete type KV, let it be going to both the daughter
			// regions.
			// No problems in doing so. In the correct daughter region where it
			// belongs to, this delete
	...
	// parent region.
	// In order to make the real seek in the HFiles, we need to build the old
	// key.
	private KeyValue getKeyPresentInHFiles(byte[] key) {
		...
		// In firstkey first 2 bytes will reperesent the key length so don't
		// consider it
		...
		if (top && 0 == keyValCopy.getValueLength() && keyValCopy.getTimestamp() == HConstants.LATEST_TIMESTAMP
				&& Bytes.compareTo(row, splitRow) == 0 && keyValCopy.isDeleteFamily()) {
			KeyValue createFirstDeleteFamilyOnRow = KeyValue.createFirstDeleteFamilyOnRow(startRow,
					keyValCopy.getFamily());
		...
		byte[] replacedKey = new byte[rowAfterSplitKey.length + afterRow.length + firstindex + ROW_KEY_LENGTH];
		...
		System.arraycopy(key, rowOffset + rowLength, afterRow, 0, (key.length - (rowOffset + rowLength)));
		...
		// Copy the actualFirstKey till firstIndex to replacedKey.. This will be
		// the start key of main
		// table
		...
		System.arraycopy(afterRow, 0, replacedKey, firstindex + rowAfterSplitKey.length + rowKeyLengthBytes.length,
				afterRow.length);
		return KeyValue.createKeyValueFromKey(replacedKey);
	}

	public byte[] getLastKey() {
		// This method wont get used for the index region. There is no need to
		// call getClosestRowBefore
		// on the index table. Also this is a split region. Can not be further
		// split
