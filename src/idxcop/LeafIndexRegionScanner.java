...
public class LeafIndexRegionScanner implements IndexRegionScanner {
	...
	public synchronized boolean next(List<KeyValue> results) throws IOException {
		...
		do {
			...
			// next last row was fetched and after that an advance was called on
			// this
			// scanner. So instead of making a next call again we can return
			// from here.
			if (!this.hadMore)
				return false;
	...
	public boolean next(List<KeyValue> result, int limit) throws IOException {
		// We wont call this method at all.. As we have only CF:qualifier per
		// row in index table.
