...
public class IndexRegionScannerForOR implements IndexRegionScanner {
  ...
  private Map<byte[], Pair<IndexRegionScanner, KeyValue>> valueMap =
      new TreeMap<byte[], Pair<IndexRegionScanner, KeyValue>>(Bytes.BYTES_COMPARATOR);
  ...
  public synchronized boolean reseek(byte[] row) throws IOException {
    ...
    if (!valueMap.isEmpty()) {
      Iterator<Entry<byte[], Pair<IndexRegionScanner, KeyValue>>> itr =
  ...
  public synchronized boolean next(List<KeyValue> results) throws IOException {
    OUTER: while (results.size() < 1) {
          ...
          if (!hasRangeScanners) {
            if (intermediateResult != null && intermediateResult.size() > 0) {
              ...
              if (!hasMore && intermediateResult.isEmpty()) {
                ...
                valueMap.put(rowKeyFromKV, new Pair<IndexRegionScanner, KeyValue>(scn,
      ...
      if (!valueMap.isEmpty()) {
        Entry<byte[], Pair<IndexRegionScanner, KeyValue>> minEntry =
  ...
  /*
   * private void getTheSmallerValue(KeyValue keyValue, IndexRegionScanner scn ) { byte[] currentRow
   * = getRowKeyFromKV(keyValue); if(this.lastReturnedValue == null){
   * this.lastReturnedValue.setFirst(new Pair<byte[], Integer>(currentRow,scn.getScannerIndex()));
   * this.lastReturnedValue.setSecond(keyValue); } else { byte[] lastRow =
   * getRowKeyFromKV(this.lastReturnedValue.getSecond()); if(Bytes.compareTo(currentRow, lastRow) <
   * 0){ this.lastReturnedValue.setFirst(new Pair<byte[],
   * Integer>(currentRow,scn.getScannerIndex())); this.lastReturnedValue.setSecond(keyValue); } //
   * TODO: In case of equal need to check what to do? if(Bytes.compareTo(currentRow, lastRow) == 0){
   * scn.advance(); } } }
   */
