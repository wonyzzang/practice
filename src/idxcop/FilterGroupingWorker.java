 ...
 * this class groups the filters lists into one filter list with all as
 * MUST_PASS_ALL. Also it checks if the combination of the filters list is given
 * correctly. For eg: If i have c1 > 10 and c1 < 5. This is wrong combination.
 */
public class FilterGroupingWorker {

	private static final Log LOG = LogFactory.getLog(FilterGroupingWorker.class);

	private Map<Column, Pair<Value, Value>> colWithOperators = new HashMap<Column, Pair<Value, Value>>();
	...
	public Filter group(Filter filter) {
		if (filter instanceof FilterList) {
			...
			// We need to create a new FL here taking up only the filters of our
			// interest
			...
			if (fList.getOperator() == Operator.MUST_PASS_ONE) {
				for (Filter subFilter : filters) {
					...
					// If result filter is not SingleColumnValueFilter or filter
					// list that means OR branch is
					// having different type of filter other than SCVF. In that
					// case we should not consider
					// the OR branch for scanning.
					if (resultFilter instanceof FilterList) {
						newFList.addFilter(resultFilter);
					} else if (resultFilter != null) {
						// This means OR filter list have at least one filter
						// other than SCVF(may be other
						// child OR branches).
						return null;
					}
				}
				...
				// AND condition as long as the condition is AND in one sub tree
				// all those can be
				// grouped under one AND parent(new one).
				for (Filter subFilter : filters) {
					Filter group = handleFilterWithinAND(subFilter);
					// group is null means, all are AND conditions and will be
					// handled at once with the
		...
		} else if (filter instanceof SingleColumnValueFilter || filter instanceof SingleColumnRangeFilter) {
			return filter;
		}
		return null;
	}
	...
	private Filter handleFilterWithinAND(Filter filter) {
		...
		} // TODO when we expose SingleColumnRangeFilter to handle that also
			// here.
		return null;
	}
	 ...
	 * create a hierarchy of filters to be evaluated. In the hierarchy if OR
	 * branch having any filter type other than SCVF as child then we should not
	 * consider the branch for scanning because we cannot fetch seek points from
	 * other type of filters without column and value details. Ex: AND AND
	 * __________|_______ | | | --> SCVF OR SCVF _______|______ | | ROWFILTER
	 * SVCF If the OR is root then we should skip index table scanning for this
	 * filter. OR _______|______ --> null | | ROWFILTER SVCF If the OR is child
	 * of another OR branch then parent OR branch will be excluded for scanning.
	 * Ex: AND AND __________|_______ | | | --> SCVF OR SCVF _______|______ | |
	 * OR SVCF _______|______ | | ROWFILTER SVCF
	 * 
	 * @param filter
	 * @return if filter is filter list with AND condition then we will return
	 *         AND branch after grouping. if filter is filter list with OR
	 *         condition return null if no children is of type other than SCVF
	 *         or filter list else return different filter. if filter is SCVF
	 *         then return null. returning null means we are combining the
	 *         filter(s) with children of parent OR filter to perform
	 *         optimizations.
	 */
	private Filter handleFilterWithinOR(Filter filter) {
		if (filter instanceof FilterList) {
			...
			if (fList.getOperator() == Operator.MUST_PASS_ONE) {
				...
				for (Filter subFilter : filters) {
					// If this OR branch in the filter list have filter type
					// other than SCVF we should report
					// it to parent by returning the other type of filter in
					// such a way that the branch will
		...
		if (colWithOperatorsOfOR.get(key) == null) {
			...
			while (valueListItr.hasNext()) {
				...
				case GREATER:
					if (prevOperator == CompareOp.GREATER || prevOperator == CompareOp.GREATER_OR_EQUAL) {
						if (result > 0) {
							...
							// Already you found less or equal value than
							// present value means present filter will
							// return subset of previous filter. No need to add
							// it to list.
							return;
						}
					} else if (prevOperator == CompareOp.LESS || prevOperator == CompareOp.LESS_OR_EQUAL) {
						// Need to handle conditions like previous is c1<5 and
						// current c1>2. In these cases we
						// can change this condition into three parts c1<2,c1>=2
						// AND C1=<5 ,c1>5 and add to
						// list.
					} else if (prevOperator == CompareOp.EQUAL) {
						if (result > 0) {
							valueListItr.remove();
						} else if (result == 0) {
							// remove this entry and convert GREATER to
							// GREATER_OR_EQUAL
							valueListItr.remove();
							SingleColumnValueFilter newScvf = null;
							if (vp == null) {
								newScvf = new SingleColumnValueFilter(scvf.getFamily(), scvf.getQualifier(),
										CompareOp.GREATER_OR_EQUAL, scvf.getComparator());
							} else {
								newScvf = new SingleColumnValuePartitionFilter(scvf.getFamily(), scvf.getQualifier(),
				...
				case GREATER_OR_EQUAL:
					if (prevOperator == CompareOp.GREATER || prevOperator == CompareOp.GREATER_OR_EQUAL) {
						if (result >= 0) {
							...
							// Already you found less value than present value
							// means present filter will
							// return subset of previous filter. No need to add
							// it to list.
							return;
						}
				...
				case LESS:
					if (prevOperator == CompareOp.LESS || prevOperator == CompareOp.LESS_OR_EQUAL) {
					...
					} else if (prevOperator == CompareOp.GREATER || prevOperator == CompareOp.GREATER_OR_EQUAL) {
						// Need to handle conditions like previous is c1<5 and
						// current c1>2. In these cases we
						// can change this condition into three parts c1<2,c1>=2
						// AND C1=<5 ,c1>5 and add to
						...
						if (result < 0) {
							...
							// remove this entry and convert LESS to
							// LESS_OR_EQUAL
							valueListItr.remove();
							SingleColumnValueFilter newScvf = null;
							if (vp == null) {
				...
				case LESS_OR_EQUAL:
					if (prevOperator == CompareOp.LESS || prevOperator == CompareOp.LESS_OR_EQUAL) {
						...
						// If we dont want to do conversion we can add into
						// first if condition.
						if (result <= 0) {
							valueListItr.remove();
						} else if (result == 0) {
	...
	private void handleScvf(SingleColumnValueFilter scvf) {
		...
		if (pair == null) {
			...
			if (pair.getFirst() != null && pair.getSecond() == null) {
				...
				case GREATER:
				case GREATER_OR_EQUAL:
					if (prevBoundOperator == CompareOp.GREATER || prevBoundOperator == CompareOp.GREATER_OR_EQUAL) {
						LOG.warn("Wrong usage.  It should be < > || > <.  Cannot be > >");
						if (result > 1) {
							pair.setFirst(new Value(curBoundOperator, curBoundValue, scvf));
						}
						pair.setSecond(null);
					} else if (prevBoundOperator == CompareOp.LESS || prevBoundOperator == CompareOp.LESS_OR_EQUAL) {
						if (result < 1) {
	...
	private void addANDColsToFinalList(FilterList filterList) {
		for (Entry<Column, Pair<Value, Value>> entry : colWithOperators.entrySet()) {
			...
			if (value.getFirst() != null && value.getSecond() != null) {
				...
				SingleColumnRangeFilter rangeFltr = new SingleColumnRangeFilter(entry.getKey().getFamily(),
						entry.getKey().getQualifier(), entry.getKey().getValuePartition(), value.getFirst().getValue(),
						value.getFirst().getOperator(), value.getSecond().getValue(), value.getSecond().getOperator());
				...
				if (value.getFirst().getOperator() == CompareOp.EQUAL) {
					...
					SingleColumnRangeFilter rangeFltr = new SingleColumnRangeFilter(entry.getKey().getFamily(),
							...
							value.getFirst().getValue(), value.getFirst().getOperator(), null, null);
					filterList.addFilter(rangeFltr);
				}
			}
		}
		...
		for (Entry<Column, List<Value>> entry : colWithOperatorsOfOR.entrySet()) {
			...
			for (Value value : valueList) {
				if (value.getOperator() == CompareOp.EQUAL) {
					...
					SingleColumnRangeFilter rangeFltr = new SingleColumnRangeFilter(entry.getKey().getFamily(),
							entry.getKey().getQualifier(), entry.getKey().getValuePartition(), value.getValue(),
							value.getOperator(), null, null);
					filterList.addFilter(rangeFltr);
				}
			}
		}
